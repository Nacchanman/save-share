...
function render() {
  ...
  if (activePage === 'important') {
    shell.append(renderPage(
      '重要',
      '←共有 / ゴミ箱→',
      renderArticleList(importantArticles, 'URLを追加しよう。', (article) => renderArticleCard({
        article,
        showMemo: true,
        leftHint: '共有へ',
        rightHint: 'ゴミ箱',
  ...
  if (activePage === 'feed') {
    ...
    content.append(renderArticleList(feedArticles, '共有記事はまだありません。', (article) => renderArticleCard({
      article,
    ...
    shell.append(renderPage('フィード', '人気順。メモは非表示。', content));
  }
  ...
  if (activePage === 'trash') {
    shell.append(renderPage(
      'ゴミ箱',
      '右スワイプで削除。',
      renderArticleList(tiredArticles, '空です。', (article) => renderArticleCard({
        article,
        showMemo: true,
        rightHint: '完全削除',
  ...
  if (activePage === 'friends') {
    shell.append(renderPage('フォロー', 'フィードに表示する友達。', renderFriends()));
  }
...
function renderHero() {
  ...
  const copy = el('div', {}, [
    ...
    el('h1', { text: 'Save links beautifully.' }),
    el('p', { className: 'hero-copy', text: 'URLを貼って、スワイプで整理。' }),
  ]);
...
function renderTabs({ importantArticles, tiredArticles, feedArticles }) {
  return el('nav', { className: 'tabs', 'aria-label': 'ページ切り替え' }, [
    ...
    tab('important', '重要', importantArticles.length),
...
function renderArticleCard({ article, actions, showMemo = false, ownerLabel, onMemo, onSwipeLeft, onSwipeRight, leftHint, rightHint }) {
  ...
  const image = el('div', { className: 'article-image' });
  image.style.backgroundImage = `linear-gradient(180deg, rgba(0,0,0,0.08), rgba(0,0,0,0.22)), url(${article.imageUrl || `https://picsum.photos/seed/${encodeURIComponent(host)}/1200/760`})`;
  if (leftHint || rightHint) {
    image.append(el('div', { className: 'swipe-hints' }, [el('span', { text: leftHint ? `← ${leftHint}` : '' }), el('span', { text: rightHint ? `${rightHint} →` : '' })]));
  }
  card.append(image);
  card.append(
